...
if [[ "$qcow_file" == *"debian"* || "$qcow_file" == *"ubuntu"* || "$qcow_file" == *"arch"* ]]; then
    ...
    sudo virt-customize -a $qcow_file --run-command "echo 'Welcome to XiYan Cloud' >> /etc/motd"
    echo "启用SSH功能..."
...
sudo virt-customize -a $qcow_file --run-command "echo root:XiYanCloudDefaultPass | chpasswd root"
sudo virt-customize -a $qcow_file --run-command "echo root:XiYanCloudDefaultPass | sudo chpasswd root"
